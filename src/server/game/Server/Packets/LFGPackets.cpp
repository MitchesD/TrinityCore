...
ByteBuffer& operator<<(ByteBuffer& data, WorldPackets::LFG::LFGBlackList const& clientBlackList)
{
    ...
    data.FlushBits();
    data << uint32(clientBlackList.Slot.size());

    if (clientBlackList.PlayerGuid)
        data << *clientBlackList.PlayerGuid;

    for (auto const& slot : clientBlackList.Slot)
    {
        data << uint32(slot.Slot);
        data << uint32(slot.Reason);
        data << uint32(slot.SubReason1);
        data << uint32(slot.SubReason2);
    }
...
ByteBuffer& operator<<(ByteBuffer& data, WorldPackets::LFG::LfgPlayerQuestReward const& playerQuestRewards)
{
    ...
    data << uint32(playerQuestRewards.Item.size());
    data << uint32(playerQuestRewards.Currency.size());
    data << uint32(playerQuestRewards.BonusCurrency.size());

    for (auto const& item : playerQuestRewards.Item)
        data << item;

    for (auto const& currency : playerQuestRewards.Currency)
        data << currency;

    for (auto const& bonusCurrency : playerQuestRewards.BonusCurrency)
        data << bonusCurrency;
...
{
    _worldPacket << int32(Dungeon.size());
    _worldPacket << BlackList;
